...
// Only the raised forearm and hand enter WebGL. Face and shoulder stay
// in the original <img>. Local preview only; not connected to the homepage.
const WIDTH = 887;
const HEIGHT = 1774;
const DURATION = 2700;
const asset = name => new URL(`assets/${name}`, import.meta.url).href;
const maskURL = asset('viktoria-profile-mask-v13.svg');
const stillMaskURL = asset('viktoria-profile-wave-body-mask-v16.svg');
...
const vertexSource = `
  ...
  void main() {
    ...
    // The thumb extends farther right than the wrist. Narrow the moving
    // region below the palm so the shoulder is completely outside the rig.
    float handSide = mix(1.0 - smoothstep(245.0, 285.0, p.x),
                         1.0 - smoothstep(205.0, 225.0, p.x),
                         smoothstep(320.0, 350.0, p.y));
    float hand = (1.0 - smoothstep(398.0, 440.0, p.y)) * handSide;
    float arm = max(hand, (1.0 - smoothstep(470.0, 615.0, p.y)) *
                         (1.0 - smoothstep(180.0, 225.0, p.x)));
...
const fragmentSource = `
  ...
  void main() {
    ...
    if (uv.y > 351.0 / 1774.0 && uv.x > 226.0 / 887.0) discard;
